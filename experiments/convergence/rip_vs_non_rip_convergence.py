...
Indeed, we also find that RIP designs have bounded primal
active curvature on sufficently sparse sets.

We test multiple design types with varying geometric properties.
Exact scaling laws e.g., RIP with n = o(s log(d/s)) are omitted for brevity, but the following designs are considered:
...
# Output directories
OUTPUT_DIR = './results/convergence/rip_vs_non_rip_convergence/'
...
plt.title('LASSO Convergence on RIP vs. Non-RIP Designs')
plt.xlabel('Iteration $k$')
plt.ylabel('Estimator Suboptimality')
plt.grid(True, which='both', alpha=0.3)
plt.legend(bbox_to_anchor=(1, 1), loc='upper left')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'test_rip_vs_non_rip_ista_convergence.png'))
