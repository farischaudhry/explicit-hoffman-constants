...
class HoffmanBoundCalculator:
    ...
    def compute_all(cls, A: np.ndarray, active_set: np.ndarray, 
        ...
        try:
            ...
            # Handle singular case (e.g., during early iterations or poor design)
            exact_hoffman = np.inf 

        # Extract blocks for explicit bound components
        G_AA_inv = None
        if s > 0:
            G_AA = G[np.ix_(active_set, active_set)] + lam2 * np.eye(s)
        ...
        # Interaction Norm Calculation
        interaction_norm = 0.0
        interaction_linf = 0.0
        interaction_l1 = 0.0
        if s > 0 and m > 0:
            G_AcA = G[np.ix_(inactive_set, active_set)]
            interaction_mat = (G_AcA @ G_AA_inv) / w_inactive[:, None]
            interaction_norm = np.linalg.norm(interaction_mat, ord=2)
            interaction_linf = np.linalg.norm(interaction_mat, ord=np.inf)
            interaction_l1 = np.linalg.norm(interaction_mat, ord=1)

        # Dimension-Free Spectral Bounds
        if s > 0:
            term_curv = np.linalg.norm(G_AA_inv, ord=2)
            term_reg = max_inv_lam
            term_inter = interaction_norm 
            
            spec_free_lower = max(term_curv, term_reg, term_inter)
            spec_free_upper = np.sqrt(term_curv**2 + term_reg**2 + term_inter**2)
        else:
            # When active set is empty, stability is purely 1/lambda
        ...
        h_inf = max(active_linf, max_inv_lam + interaction_linf)

        active_l1 = np.linalg.norm(G_AA_inv, ord=1) if s > 0 else 0.0
        h_1 = max(active_l1 + interaction_l1, max_inv_lam)
