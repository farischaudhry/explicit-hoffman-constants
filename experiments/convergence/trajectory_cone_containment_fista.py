"""
This experiment investigates the key motivation for trajectory-based Hoffman constants:
    Do FISTA iterates stay within the restricted cone C_α(Â)?
...
In this experiment, we init β^(0) = 0. 
...
from hoffman.solvers.lasso_solver import FISTALassoSolver
from hoffman.utils.viz import set_plotting_style

# Output directories
OUTPUT_DIR = './results/convergence/trajectory_cone_containment_fista/'
...
def track_trajectory(design, lam: float, max_iters: int):
    """Run FISTA and track detailed trajectory information."""
    ...
    # Run FISTA solver with trajectory tracking
    solver = FISTALassoSolver(design, lam=lam, y=y, beta_hat=beta_star)
