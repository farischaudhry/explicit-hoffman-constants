...
class HoffmanBoundMetrics:
    ...
    # Calculated with SVD methods for ||M_A^{-1}||_p
    exact_hoffman: float
    ...
    interaction_norm_p: float = 0.0  # ||G_{A^c A}||_p
...
class HoffmanBoundCalculator:
    ...
    @staticmethod
    def _compute_hoffman_max_bounds(G: np.ndarray, active_set: np.ndarray, lam: float) -> tuple[float, float, dict]:
        """
        ...
        if k == 0:
            ...
            return 1.0/lam, 1.0/lam, {'lambda_min_AA': np.nan, 'interaction': 0.0}
        
        # Extract blocks
        G_AA = G[np.ix_(active_set, active_set)]
        
        # Minimum eigenvalue of active block
        lambda_min_AA = np.linalg.eigvalsh(G_AA).min()
        
        if lambda_min_AA <= 1e-15 or np.isnan(lambda_min_AA):
            # Singular active block
            return np.inf, np.inf, {'lambda_min_AA': lambda_min_AA, 'interaction': 0.0}
        ...
        if m > 0:
            G_AcA = G[np.ix_(inactive_set, active_set)]
            interaction_norm = np.linalg.norm(G_AcA, ord=2)
            
            # Upper bound: max(1/κ, (1/λ)(1 + ||G_AcA||/κ))
            term1 = inv_kappa
            term2 = (1.0 / lam) * (1.0 + interaction_norm / lambda_min_AA)
        ...
        components = {
            ...
            'interaction_norm_p': interaction_norm,
        }
    ...
    def compute_all(cls, G: np.ndarray, active_set: np.ndarray, lam: float, A: np.ndarray | None = None) -> HoffmanBoundMetrics:
        ...
        Parameters:
            ...
            A: Original design matrix (optional, for leverage scores)
        """
        exact = cls.compute_exact_hoffman(G, active_set, lam)
        lower, upper, comps = cls._compute_hoffman_max_bounds(G, active_set, lam)
        ...
        return HoffmanBoundMetrics(
            ...
            interaction_norm_p=comps['interaction_norm_p'],
